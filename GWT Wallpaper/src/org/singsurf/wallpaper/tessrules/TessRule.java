...
import org.singsurf.compat.Component;
...
//        if(DEBUG) System.out.println("lattice "+latticeWidth+" "+latticeHeight);
	...
	GWT.log("Range "+startX+" "+startY+" "+baseRect.width+" "+baseRect.height);
        
        int stopX = startX+baseRect.width;
	int stopY = startY+baseRect.height;
	    ...
	    for(int j=startY;j<stopY;++j)
            {
                ...
                try
                {
                    int px_r,px_g,px_b,px_a;
                        ...
                        int inInd = (srcX+srcY*dr.srcRect.width)*4;
                        px_r = dr.inCPA.get(inInd);
                        px_g = dr.inCPA.get(inInd+1);
                        px_b = dr.inCPA.get(inInd+2);
                        px_a = dr.inCPA.get(inInd+3);

                        //if(i==0&&j==0) px = backgroundRGB;
                    int outX = i;
                    int outY = j;
                    int outInd = (outX+outY*dr.destRect.width)*4;
                    
                    dr.outCPA.set(outInd  ,px_r);
                    dr.outCPA.set(outInd+1,px_g);
                    dr.outCPA.set(outInd+2,px_b);
                    dr.outCPA.set(outInd+3,px_a);
                ...
                catch(Exception e)
                {
                    ...
                    error_flag = true;
                    int outInd = (i+j*dr.destRect.width)*4;
                    dr.outCPA.set(outInd  ,0);
                    dr.outCPA.set(outInd+1,0);
                    dr.outCPA.set(outInd+2,0);
                    dr.outCPA.set(outInd+3,0);
                }
            }
	}
        
        long t2 = System.currentTimeMillis();
        if(TIME) Wallpaper.consoleLog("Replicate "+((t2-t1)) +"ms");
        ...
        if(TIME) Wallpaper.consoleLog("CopyTiles "+((t3-t2)) +"ms");
...
//                        resRect.width);
            	int inBase = 4*(sx + (sy+k) * dr.destRect.width);
            	int outBase = 4*(dx+(dy+k) * dr.destRect.width);
            	
            	for(int j=0;j<resRect.width*4;++j) {
                        int px = dr.outCPA.get(inBase+j);
                        dr.outCPA.set(outBase+j, px);
            	}
            }
        }
    }
