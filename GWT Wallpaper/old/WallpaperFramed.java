...
//            }
        ...
        protected ItemListener viewActionListener = new ItemListener() {
            ...
            public void itemStateChanged(ItemEvent e) {
                ...
                if(label.equals("l"))
                    fd.drawCells = cbmi.getState();
                if(label.equals("m"))
                    fd.drawReflectionLines = cbmi.getState();
                if(label.equals("r"))
                    fd.drawRotationPoints = cbmi.getState();
                if(label.equals("g"))
                    fd.drawGlideLines = cbmi.getState();
                if(label.equals("d"))
                    fd.drawDomain = cbmi.getState();
                if(label.equals("s"))
                    fd.drawSelectionPoints = cbmi.getState();
                if(label.equals("t"))
                    fd.drawTiles = cbmi.getState();
                if(label.equals("a")) {
                ...
                for(int i=0;i<viewMenu.getItemCount();++i) {
                    ...
                    if("a".equals(item.getActionCommand())) {
                        ((JCheckBoxMenuItem)item).setState(all);
                    }
                    if("h".equals(item.getActionCommand())) {
                        ((JCheckBoxMenuItem)item).setState(hide);
                    }
                }
